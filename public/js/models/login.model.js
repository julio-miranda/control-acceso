...
(function (window) {
  ...
  function normalizeRole(role) {
    if (window.RolePolicy?.normalizeRole) return window.RolePolicy.normalizeRole(role);
  ...
  function getRedirectByRole(role) {
    if (window.RolePolicy?.getRedirectByRole) {
      return window.RolePolicy.getRedirectByRole(role);
    }

    const normalized = normalizeRole(role);

    if (normalized === "admin") return "dashboard.html";
    if (normalized === "barra") return "sales.html";
    if (normalized === "bodega") return "inventory.html";
    if (normalized === "empleado") return "employee.html";
